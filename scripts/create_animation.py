...
def animate_driver(contour_file_path, sampler, dim1=0, dim2=1, X=None, Y=None, Z=None):
    """
    ...
    animation_name = 'particle_flow_%i_%i.gif' % (dim1, dim2)
    ...
    # plt.axis('off')
    # cp = ax.contourf(X, Y, Z, 7, colors='black', alpha=0.1)
    ...
    with h5py.File(sampler.history_path, 'r') as hf:
        def update(i):
            ...
            # index = np.arange(iterationsPerformed - 500, iterationsPerformed, 1) # Use to animate last 'x' iterations
            j = index[i]
            if i < iterationsPerformed:
                particles_x_i = hf['%i' % j]['X'][:, dim1]
                particles_y_i = hf['%i' % j]['X'][:, dim2]
            else:
                particles_x_i = hf['final_updated_particles']['X'][:, dim1]
                particles_y_i = hf['final_updated_particles']['X'][:, dim2]
