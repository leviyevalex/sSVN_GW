...
def psichichia_from_chi1chi2(chi1, chi2, eta):
    Seta = jnp.sqrt(jnp.where(eta<0.25, 1.0 - 4.0*eta, 0.))
    psichi = (113.*Seta*(chi1-chi2) + (chi1+chi2)*(113.-76.*eta) - 96*jnp.pi)/(256.*eta**(3./5.))
    chia = 0.5*(chi1 - chi2)

    return psichi, chia

def chi1chi2_from_psichichia(psichi, chia, eta):
    Seta = jnp.sqrt(jnp.where(eta<0.25, 1.0 - 4.0*eta, 0.))

    chi2 = (48.*np.pi + 128.*(eta**(3./5.))*psichi - 113.*chia*Seta)/(113. - 76.*eta) - chia
    chi1 = 2.*chia + chi2
...
def GMST_to_LMST(tGMST, lat, long):
    ...
    """
    return jnp.remainder(tGMST.real + 1./15.*long/24., 1.)
...
def LMST_to_GMST(tLMST, lat, long):
    ...
    """
    return jnp.remainder(tLMST.real - 1./15.*long/24., 1.)
