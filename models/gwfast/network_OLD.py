...
from models.gwfast.signal import GWSignal
...
class LV_DetNet(object):
    """
    Class to build a network of LIGO-Virgo detectors.
    ...
    """
    def __init__(self, wf_model, fixed_fgrid=None, 
                 ASDs={'L1':'/home/al44828/projects/sSVN_GW/notebooks/aLIGO_O4_high_asd.txt', 'H1':'/home/al44828/projects/sSVN_GW/notebooks/aLIGO_O4_high_asd.txt', 'Virgo':'/home/al44828/projects/sSVN_GW/notebooks/AdV_asd.txt'}, 
                 verbose=False):
        """
        Constructor method
        """
        ...
        self.fmin = min(self.fixed_fgrid)
        self.fmax = max(self.fixed_fgrid)
        self.wf_model = wf_model

        signals = {}
        signals['L1'] = GWSignal(wf_model, 
                                 psd_path       = os.path.join(glob.detPath, ASDs['L1']),
                                 detector_shape = glob.detectors['L1']['shape'],
        ...
        signals['H1'] = GWSignal(wf_model,
                                 psd_path       = os.path.join(glob.detPath, ASDs['H1']),
                                 detector_shape = glob.detectors['H1']['shape'],
        ...
        # signals['Virgo'] = GWSignal(wf_model,
        #                             psd_path       = os.path.join(glob.detPath, ASDs['Virgo']),
        #                             detector_shape = glob.detectors['Virgo']['shape'],
    ...
    def GWstrain(self, Mc, eta, dL, theta, phi, iota, psi, tcoal, Phicoal, chi1z, chi2z):
        """
        ...
        def single_signal(d):
            ...
            return (sig_p_tmp + sig_c_tmp)*np.exp(1j*(phiL + 2.*np.pi*fgrids*(tcoal*3600.*24.) - Phicoal))
    ...
    def SNR(self, evParams, res=1000):
        ...
        def tmpSNR(d, totSNR):
            strainGrids = np.interp(fgrids, self.signals[d].strainFreq, self.signals[d].noiseCurve, left=1., right=1.)
            Fp, Fc = self.signals[d]._PatternFunction(evParams['theta'], evParams['phi'], evParams['tcoal'], evParams['psi'], rot=0.)
            Atot = Ap*Ap*Fp*Fp + Ac*Ac*Fc*Fc
            return totSNR + 4.*np.trapz(Atot/strainGrids, fgrids, axis=0)

        totSNR = tmpSNR('L1', totSNR)
        totSNR = tmpSNR('H1', totSNR)
        # totSNR = tmpSNR('Virgo', totSNR)
    ...
    def FisherMatr(self, evParams):
        ...
        FisherDerivs.at[tcelem].divide(3600.*24.)
        Fisher = Fisher + tmpFisher('L1')
        Fisher = Fisher + tmpFisher('H1')
        # Fisher = Fisher + tmpFisher('Virgo')
    ...
    def _SignalDerivatives(self, Mc, eta, dL, theta, phi, iota, psi, tcoal, Phicoal, chi1z, chi2z):
        ...
        allDerivs_Fish['L1'] = singleFish('L1')
        allDerivs_Fish['H1'] = singleFish('H1')
        # allDerivs_Fish['Virgo'] = singleFish('Virgo')
