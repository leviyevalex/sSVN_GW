...
class WaveFormModel(ABC):
    ...
    def __init__(self, objType, fcutPar, is_holomorphic=False):
        """
        Constructor method
        """
        ...
        self.ParNums = {'Mc':0, 'eta':1, 'chi1z':2,  'chi2z':3, 'dL':4, 'theta':5, 'phi':6, 'iota':7, 'psi':8, 'tcoal':9, 'Phicoal':10}
...
class TaylorF2_RestrictedPN(WaveFormModel):
    ...
    def __init__(self, fHigh=None):
        ...
        objectT = 'BBH'
        super().__init__(objectT, fHigh, is_holomorphic=True)
    ...
    def Phi(self, f, **kwargs):
        ...
        TF2coeffs_zero = 1.
        TF2coeffs_one = 0.
        TF2coeffs_two = 3715./756. + (55.*eta)/9.
        TF2coeffs_three = -16.*np.pi + (113.*Seta*chi_a)/3. + (113./3. - (76.*eta)/3.)*chi_s
        TF2coeffs_four = 15293365./508032. + (27145.*eta)/504.+ (3085.*eta2)/72. + (-405./8. + 200.*eta)*chi_a2 - (405.*Seta*chi_sdotchi_a)/4. + (-405./8. + (5.*eta)/2.)*chi_s2
        ...
        TF2coeffs_five = (38645.*np.pi/756. - 65.*np.pi*eta/9. - TF2_5coeff_tmp)
        # This pi factor is needed to include LAL fRef rescaling, so to end up with the exact same waveform
        phiR = np.pi
        TF2coeffs_five_log = (38645.*np.pi/756. - 65.*np.pi*eta/9. - TF2_5coeff_tmp)*3.
        TF2coeffs_six = 11583231236531./4694215680. - 640./3.*np.pi**2 - 6848./21.*np.euler_gamma + eta*(-15737765635./3048192. + 2255./12.*np.pi**2) + eta2*76055./1728. - eta2*eta*127825./1296. - (6848./21.)*np.log(4.) + np.pi*(2270.*Seta*chi_a/3. + (2270./3. - 520.*eta)*chi_s) + (75515./144. - 8225.*eta/18.)*Seta*chi_sdotchi_a + (75515./288. - 263245.*eta/252. - 480.*eta2)*chi_a2 + (75515./288. - 232415.*eta/504. + 1255.*eta2/9.)*chi_s2
        TF2coeffs_six_log = -(6848./21.)
        TF2coeffs_seven = 77096675.*np.pi/254016. + 378515.*np.pi*eta/1512.- 74045.*np.pi*eta2/756. + (-25150083775./3048192. + 10566655595.*eta/762048. - 1042165.*eta2/3024. + 5345.*eta2*eta/36.)*chi_s + Seta*((-25150083775./3048192. + 26804935.*eta/6048. - 1985.*eta2/48.)*chi_a)
            
        phase = TF2OverallAmpl*(TF2coeffs_zero/(v**5) + TF2coeffs_one/(v**4) + TF2coeffs_two/(v*v*v) + TF2coeffs_three/(v*v) + TF2coeffs_four/v + (TF2coeffs_five + TF2coeffs_five_log*np.log(v)) + (TF2coeffs_six + TF2coeffs_six_log*np.log(v))*v + TF2coeffs_seven*v**2)
...
class IMRPhenomD(WaveFormModel):
    ...
    def __init__(self, fRef=20., **kwargs):
        """
        Constructor method
        """
    ...
    def Phi(self, f, **kwargs):
        ...
        #logfgrid = np.log(fgrid)
        #logpifgrid = np.log(np.pi*fgrid)
        #C2Intfgrid = C2Int*fgrid + C1Int
        #phase_insp   = PhiInspcoeffs['initial_phasing'] + PhiInspcoeffs['two_thirds']*(fgrid**(2./3.)) + PhiInspcoeffs['third']*(fgrid**(1./3.)) + PhiInspcoeffs['third_log']*(fgrid**(1./3.))*logpifgrid/3. + PhiInspcoeffs['log']*logpifgrid/3. + PhiInspcoeffs['min_third']*(fgrid**(-1./3.)) + PhiInspcoeffs['min_two_thirds']*(fgrid**(-2./3.)) + PhiInspcoeffs['min_one']/fgrid + PhiInspcoeffs['min_four_thirds']*(fgrid**(-4./3.)) + PhiInspcoeffs['min_five_thirds']*(fgrid**(-5./3.)) + (PhiInspcoeffs['one']*fgrid + PhiInspcoeffs['four_thirds']*(fgrid**(4./3.)) + PhiInspcoeffs['five_thirds']*(fgrid**(5./3.)) + PhiInspcoeffs['two']*fgrid*fgrid)/eta
        #phase_interm = (beta1*fgrid - beta3/(3.*fgrid**3) + beta2*logfgrid)/eta + C2Intfgrid
        #phaser_pm    = (-(alpha2/fgrid) + (4.0/3.0) * (alpha3 * (fgrid**(3./4.))) + alpha1 * fgrid + alpha4 * np.arctan((fgrid - alpha5 * fring)/fdamp))/eta + C1MRD + C2MRD*fgrid
        ...
        #phis = np.where(fgrid < self.PHI_fJoin_INS, phase_insp, np.where(fgrid<fMRDJoin, phase_interm, phaser_pm))
    ...
    def Ampl(self, f, **kwargs):
        ...
        #ampl_insp   = 1. + (fgrid**(2./3.))*Acoeffs['two_thirds'] + (fgrid**(4./3.)) * Acoeffs['four_thirds'] + (fgrid**(5./3.)) *  Acoeffs['five_thirds'] + (fgrid**(7./3.)) * Acoeffs['seven_thirds'] + (fgrid**(8./3.)) * Acoeffs['eight_thirds'] + fgrid * (Acoeffs['one'] + fgrid * Acoeffs['two'] + fgrid*fgrid * Acoeffs['three'])
        #ampl_interm = delta0 + fgrid*delta1 + fgrid*fgrid*(delta2 + fgrid*delta3 + fgrid*fgrid*delta4)
        #ampl_pm     = np.exp(-(fgrid - fring)*gamma2/(fdamp*gamma3))* (fdamp*gamma3*gamma1) / ((fgrid - fring)*(fgrid - fring) + fdamp*gamma3*fdamp*gamma3)
        ...
        #amplitudeIMR = np.where(fgrid < self.AMP_fJoin_INS, ampl_insp, np.where(fgrid < fpeak, ampl_interm, ampl_pm))
