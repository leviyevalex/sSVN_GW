...
class gwfast_LVGW150914(object):
    ...
    def __init__(self, wf_model=TaylorF2_RestrictedPN, fmin=20., fmax=512.):
            ...
            self.fgrid, self.htrue['L1'], asd_compute['L1'] = self.get_data_from_gwpy(ifo='L1', gps_time=tGPS[0], duration=duration, post_trigger_duration=post_trigger_duration, roll_off=0.2, roll_off_psd=0.4)
            _, self.htrue['H1'], asd_compute['H1']          = self.get_data_from_gwpy(ifo='H1', gps_time=tGPS[0], duration=duration, post_trigger_duration=post_trigger_duration, roll_off=0.2, roll_off_psd=0.4)
    ...
    def get_data_from_gwpy(self, ifo, gps_time, duration=4, post_trigger_duration=2, roll_off=0.2, roll_off_psd=0.4, psd_extra_duration=32):
        ...
        data = data_freq[(freq > self.fmin) & (freq < self.fmax)] * jnp.exp(-1j * 2 * jnp.pi * frequencies * self.reftime * 3600. * 24.)
