...
from src.birth_death_GV import birth_death
...
def ula_kernel(key, X, potential, grad_potential, dt, iteration, lower, upper, stride, rate, sigmas, bounded_coordinates, periodic_coordinates):
    ...
    centered = gmlpt_Z - jnp.mean(gmlpt_Z, axis=0)
    damping = 0.001 # 0.001
    preconditioner = jnp.mean(centered[:,:,None] * centered[:,None,:], axis=0) + damping * jnp.eye(d)
    U = cholesky(preconditioner, lower=False)
    U = jnp.repeat(U[None, ...], N, axis=0)

    sigmas_new = jnp.sqrt(1 / jnp.diag(preconditioner)) * sigmas

    # jumps = jax.lax.cond(jnp.mod(iteration, stride) == 0, lambda: birth_death(subkey, X, potential, rate=rate, a=lower, b=upper, bounded_coordinates=bounded_coordinates, periodic_coordinates=periodic_coordinates, sigmas=sigmas, gmlpt_X=gmlpt_X), lambda: jnp.arange(N))
    jumps = jax.lax.cond(jnp.mod(iteration, stride) == 0, lambda: birth_death(subkey, X, potential, rate=rate, a=lower, b=upper, bounded_coordinates=bounded_coordinates, periodic_coordinates=periodic_coordinates, sigmas=sigmas_new), lambda: jnp.arange(N))

    Z = Z[jumps]
    Z += -cho_solve((U, False), gmlpt_Z[jumps]) * dt + jnp.sqrt(2 * dt) * solve_triangular(U, jax.random.normal(key=subkey, shape=(X.shape)), lower=False)
    # Z += -gmlpt_Z[jumps] * dt + jnp.sqrt(2 * dt) * jax.random.normal(key=subkey, shape=X.shape) # Regular Langevin 
